...
// ========== EXISTING CHATBOT FUNCTIONALITY ==========
      ...
      async function handleStreamingResponse(response) {
        ...
        try {
            ...
            for (const line of lines) {
              ...
              try {
                ...
                if (data.type === 'start') {
                  ...
                  messageElement = createStreamingMessageElement();
                } else if (data.type === 'chunk' && data.content) {
                  accumulatedResponse += data.content;
                  if (messageElement) {
                    updateStreamingMessage(messageElement, accumulatedResponse);
                  }
                } else if (data.type === 'end') {
                  // Streaming complete
                  if (messageElement) {
                    finalizeStreamingMessage(messageElement, accumulatedResponse);
                  }
                }
              } catch (e) {
          ...
          if (messageElement) {
            addMessage("Sorry, an error occurred while streaming the response.", "bot");
          }
        }
      }
      ...
      function finalizeStreamingMessage(element, text) {
        ...
        const parsed = parseResponseForSuggestions(text);
