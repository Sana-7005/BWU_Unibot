...
def chat():
    ...
    # Check admin backend to see if chatbot is enabled (optional feature)
    ...
    if enable_admin:
        try:
            ...
            # Admin backend unreachable - continue with chatbot (fail-open)
    ...
    # Default to non-streaming because the frontend expects JSON, not SSE
    stream_enabled = data.get("stream", False)
    
    if stream_enabled:
        def generate():
            # Send intent first
            yield f"data: {json.dumps({'intent': intent, 'type': 'start'})}\n\n"
...
if __name__ == "__main__":
    ...
    if enable_admin:
        ...
        print(f"   URL: {os.getenv('ADMIN_PANEL_URL', 'http://localhost:8000')}")
    else:
        print(f"🔧 Admin Panel: DISABLED (Running standalone)")
    ...
    print("=" * 60)
    print("✅ Chatbot is running independently")
    print("💡 To enable admin panel, set ENABLE_ADMIN_PANEL=true in .env")
    print("=" * 60)
